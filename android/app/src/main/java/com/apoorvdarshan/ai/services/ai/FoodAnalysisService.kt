...
class FoodAnalysisService(
    ...
    private suspend fun inferServingUnitOptions(
        ...
        imageBytes: ByteArray?,
        description: String?
    ): List<com.apoorvdarshan.ai.models.ServingUnitOption> {
    ...
    private suspend fun dispatch(
        ...
        return when (provider.apiFormat) {
            AIProvider.ApiFormat.GEMINI ->
                GeminiClient.analyze(okHttp, baseUrl, model, apiKey!!, prompt, imageBytesList.firstOrNull())
            AIProvider.ApiFormat.ANTHROPIC ->
                AnthropicClient.analyze(okHttp, baseUrl, model, apiKey!!, prompt, imageBytesList.firstOrNull())
            AIProvider.ApiFormat.OPENAI_COMPATIBLE ->
                OpenAICompatibleClient.analyze(okHttp, baseUrl, model, apiKey, prompt, imageBytesList.firstOrNull(), provider)
        }
    }
